...
void EbpfProcessMapsTable::FilterByConstraint(FilterConstraints& profc,
                                              double& profilterCost,
                                              size_t prorowCount,
                                              uint32_t procurrenti)
{
    // To use the EstimateFilterCost function in the TableBase parent class function to calculate the i-value of each
    // for loop
    const auto& proc = profc.GetConstraints()[procurrenti];
    switch (static_cast<Index>(proc.col)) {
        case Index::ID: {
            if (CanFilterId(proc.op, prorowCount)) {
                profc.UpdateConstraint(procurrenti, true);
                profilterCost += 1; // id can position by 1 step
            } else {
                profilterCost += prorowCount; // scan all rows
            }
            break;
        }
        default:                            // other column
            profilterCost += profilterCost; // scan all rows
            break;
    }
}
...
void EbpfProcessMapsTable::GetOrbyes(FilterConstraints& profc, EstimatedIndexInfo& proei)
{
    auto proorderbys = profc.GetOrderBys();
    for (auto i = 0; i < proorderbys.size(); i++) {
        switch (static_cast<Index>(proorderbys[i].iColumn)) {
            case Index::ID:
                break;
            default: // other columns can be sorted by SQLite
                proei.isOrdered = false;
                break;
        }
    }
}
