...
import { HiPerfCpuStruct } from '../../../../src/trace/database/ui-worker/ProcedureWorkerHiPerfCPU';
import { hiperfCpuDataSender } from '../../../../src/trace/database/data-trafic/HiperfCpuDataSender';
...
describe('HiperfCpuDataSender Test',()=>{
  ...
  it('HiperfCpuDataTest01 ', function () {
    threadPool.submitProto = jest.fn((query: number, params: any, callback: Function) => {
      callback(HiperfCpuData, HiperfCpuData.length, true);
    });
   let cpu = -1;
   let drawType = -2;
   let maxCpuCount = 4;
