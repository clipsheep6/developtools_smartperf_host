...
export class CmdConstant {
  ...
  static CMD_GET_DEBUG_PROCESS = `hdc_std shell netstat -anp 
  |grep Panda |grep -v grep | sed \'s/.* \\([0-9]*\\)\\/.*/\\1/\' |xargs -r ps -A -opid,cmd`;
  ...
  static CMS_HDC_STOP =
    'hdc_std -t {0} shell killall -2 hiprofiler_cmd';
