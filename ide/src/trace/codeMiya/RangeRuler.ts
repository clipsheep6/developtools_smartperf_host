...
export class RangeRuler extends Graph {
  ...
  pressFrameIdSliceIntoView: number = -1;
  upFrameIdW: number = -1;
  upFrameIdS: number = -1;
  upFrameIdA: number = -1;
  upFrameIdD: number = -1;
  ...
  cancelPressFrame(): void {
    ...
    if (this.pressFrameIdSliceIntoView !== -1) cancelAnimationFrame(this.pressFrameIdSliceIntoView);
  }
  ...
  scrollSliceIntoView(): void {
    let animSliceIntoView = () => {
      let clientWidth = this.canvas?.clientWidth || 0;
      let sliceStartTime: number = this.currentSlicesTime.startTime!;
      let sliceEndTime: number = this.currentSlicesTime.endTime!;
      let unitValue = Number(((this.range.endNS - this.range.startNS) / clientWidth).toFixed(2));
      if (sliceEndTime > this.range.endNS) {
        let offsetNs = 10 * unitValue + sliceEndTime - this.range.endNS;
        this.range.startNS += offsetNs;
        this.range.endNS += offsetNs;
      } else if (sliceStartTime < this.range.startNS) {
        let offsetNs = this.range.startNS - sliceStartTime + 10 * unitValue;
        this.range.startNS -= offsetNs;
        this.range.endNS -= offsetNs;
      }
      this.fillX();
      this.draw();
      this.range.refresh = false;
      this.pressFrameIdSliceIntoView = requestAnimationFrame(animSliceIntoView);
    };
    this.pressFrameIdSliceIntoView = requestAnimationFrame(animSliceIntoView);
  }
