...
export class ProcedureLogicWorkerNativeMemory extends LogicHandler {
  ...
  formatUnknownString(input: string): string {
    const hexValue = input.slice(8);
    const realHexValue = `${hexValue.slice(0,2)}${hexValue.slice(8)}`;
    const decimalValue = parseInt(realHexValue, 16);
    const formattedString = `alloc size(${decimalValue}bytes) ${hexValue}`;
  ...
  setMerageName(currentNode: NativeHookCallInfo): void {
    ...
    if (currentNode.symbol.startsWith('unknown') && currentNode.symbol !== 'unknown'){
      currentNode.symbol = this.formatUnknownString(currentNode.symbol);
    }
