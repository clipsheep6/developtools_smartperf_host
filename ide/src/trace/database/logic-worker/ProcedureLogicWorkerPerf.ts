...
export class ProcedureLogicWorkerPerf extends LogicHandler {
  ...
  private perfAsync(data: any): void {
    if (data.params.list) {
      ...
      let arr = convertJSON(data.params.list) || [];
      // this.dataCache.nmHeapFrameMap
      // merageBeanDataSplit
      dealAsyncData(arr, this.callChainData, this.dataCache.nmHeapFrameMap)
      self.postMessage({
        id: data.id,
        action: data.action,
        results: arr,
    ...
    from 
      perf_napi_async A, trace_range R
		...
		ON 
			eventTypeId = c.id
    WHERE 
      tid in (${args.tid.join(',')})
    AND
      time between ${args.leftNs} and ${args.rightNs} 
    group by tid,caller_callchainid,callee_callchainid,event_type_id,traceid
    `, {});
  }
