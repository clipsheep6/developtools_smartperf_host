...
export class SampleRender extends Render {
  renderMainThread(
    ...
    if (!funcFind && row.isHover) SampleStruct.hoverSampleStruct = undefined;
...
export function func (
  ...
  if (sampleList) {
    sampleList.forEach(func => {
      ...
      .reduce((pre, current) => {
        ( pre[`${current.frame.x}-${current.level}`] = pre[`${current.frame.x}-${current.level}`] || []).push(current);
        return pre;
      }, {});
...
export class SampleStruct extends BaseStruct {
  ...
  static draw(ctx: CanvasRenderingContext2D, data: SampleStruct) {
