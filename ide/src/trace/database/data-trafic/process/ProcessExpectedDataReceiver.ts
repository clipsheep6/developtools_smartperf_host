...
export const chartProcessExpectedDataSql = (args: unknown): string => {
  return `
  SELECT 
         (a.ts - ${//@ts-ignore
          args.recordStartNS}) AS ts,
         a.dur,
         ${//@ts-ignore
          args.pid} as pid,
         a.id,
  ...
  WHERE a.type = 1
    ...
    and a.ipid in (select p.ipid from process AS p where p.pid = ${//@ts-ignore
      args.pid})
  ORDER BY a.ipid`;
};

export const chartProcessExpectedProtoDataSql = (args: unknown): string => {
  return `
  SELECT 
         (a.ts - ${//@ts-ignore
          args.recordStartNS}) AS ts,
         ...
         a.depth,
         (a.ts - ${//@ts-ignore
          args.recordStartNS}) / (${Math.floor((args.endNS - args.startNS) / args.width)}) + (a.depth * ${//@ts-ignore
    args.width
  })  AS px
  FROM frame_slice AS a
  WHERE a.type = 1
    ...
    and a.ipid in (select p.ipid from process AS p where p.pid = ${//@ts-ignore
      args.pid})
    and (a.ts - ${//@ts-ignore
      args.recordStartNS} + a.dur) >= ${Math.floor(args.startNS)}
    and (a.ts - ${//@ts-ignore
...
export function processExpectedDataReceiver(data: unknown, proc: Function): void {//@ts-ignore
  if (data.params.trafic === TraficEnum.Memory) {//@ts-ignore
    if (!processFrameList.has(`${data.params.pid}_expected`)) {//@ts-ignore
