...
export const sqlMem = (args: unknown): string => {
  return `select B.cpu, B.dur AS dur, B.itid AS id, B.tid AS tid, B.state, B.pid, B.ts - ${//@ts-ignore
...
export function threadDataReceiver(data: unknown, proc: Function): void {//@ts-ignore
  if (data.params.trafic === TraficEnum.Memory) {//@ts-ignore
    let key = `${data.params.pid}-${data.params.tid}`;
    if (!threadStateList.has(key)) {//@ts-ignore
