...
export const queryCallerStack = (arr: Array<number>): Promise<Array<{ depth: number; name: number; data: string }>> =>
  query(
    'queryCallerStack', 
    `
      select 
        A.callchain_id,A.depth,B.data
      from
        perf_callchain A, data_dict B
      where
        A.callchain_id in (${arr.join(',')})
      and 
        A.name = B.id
    `
  );
export const queryCalleeStack = (arr: Array<number>): Promise<Array<{ depth: number; name: number; data: string }>> =>
  query(
    'queryCalleeStack', 
    `
      select 
        A.callchain_id,A.depth,B.data
      from
        native_hook_frame A, data_dict B
      where
        A.callchain_id in (${arr.join(',')})
      and 
        A.symbol_id = B.id
