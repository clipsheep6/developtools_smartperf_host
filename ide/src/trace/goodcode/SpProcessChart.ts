...
export class SpProcessChart {
  ...
  hanldCategoryAsyncFunc(
    ...
    let asyncCatMap: Map<string, Array<any>> = new Map<string, Array<any>>();
  ...
  hanldAsyncFunc(
    ...
    Reflect.ownKeys(asyncFuncGroup).map((key: any) => {//@ts-ignore
      let asyncFunctions: Array<any> = asyncFuncGroup[key];
      if (asyncFunctions.length > 1) {//@ts-ignore
    ...
    Reflect.ownKeys(asyncFuncTidGroup).map((key: any) => {
      //@ts-ignore
      let asyncTidFunc: Array<any> = asyncFuncTidGroup[key];//@ts-ignore
      ...
      while (i < asyncFunctions.length) {
        ...
        if (param.dur !== null) {
          ...
          for (let [key, val] of mapDepth.entries()) {
            if (val.item < param.startTs) {
              itemi = true;
              val.item = itemEndTime;//更新endts
          ...
          if (!itemi) {
            maxDepth = maxDepth + 1;
            mapDepth.set(`${maxDepth}`, { item: itemEndTime });
