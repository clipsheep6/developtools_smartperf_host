...
import { SampleStruct, SampleRender, CountRender, CountStruct } from '../../database/ui-worker/ProcedureWorkerBpftrace';
        ...
        row.uploadEl?.addEventListener('sample-file-change', (e: unknown) => {
            this.getJsonData(e).then((res: unknown) => {
                ...
                if (!res) {
                    ...
                    //@ts-ignore
                    const propertyData = res.data;
                    let sampleProperty: Array<unknown> = [];
                    //@ts-ignore
                    SpUserFileChart.userPluginData = res.data[0];
                    //@ts-ignore
                    if (res.relation.type === "NoDraw") {
                        //@ts-ignore
                        const treeNodes = res.relation.children || [res.relation.RS.children[0]];
                        const uniqueProperty = this.removeDuplicates(propertyData);
                        const flattenTreeArray = this.getFlattenTreeData(treeNodes);
                        //@ts-ignore
                        const height = (Math.max(...flattenTreeArray.map((obj: unknown) => obj.depth)) + 1) * 20;
                        sampleProperty = this.setRelationDataProperty(flattenTreeArray, uniqueProperty);
                        //@ts-ignore
                        ...
                        //@ts-ignore
                    } else if (res.relation.type === "Count") {
                        row.focusHandler = (): void => {
                            this.trace?.displayTip(
                                row,
                                CountStruct.hoveCountStruct,
                              `<span>${ColorUtils.formatNumberComma(CountStruct.hoveCountStruct?.count!)}</span>`
                            );
                          };
                        row.style.height = '40px';
                        let currentMaxValue = 0;
                        //@ts-ignore
                        sampleProperty = res.data[0];
                        //@ts-ignore
                        sampleProperty.map((item: CountStruct) => {
                            item.dur = item.end! - item.begin!;
                          if (item.count! > currentMaxValue) {
                            currentMaxValue = item.count!;
                          }
                        });
                        CountStruct.maxCount = currentMaxValue;
                        //@ts-ignore
                        const startTS = start_ts > 0 ? start_ts : res.data[0][0].begin;
                        row.supplier = () =>
                            new Promise((resolve): void => {
                                resolve(sampleProperty)
                            })
                        row.onThreadHandler = (useCache) => {
                            ...
                            row.canvasSave(context);
                            (renders.count as CountRender).renderMainThread(
                                {
                                    context: context,
                                    useCache: useCache,
                                    type: 'bpftrace',
