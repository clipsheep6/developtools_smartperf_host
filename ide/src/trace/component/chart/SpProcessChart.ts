      ...
      if (allJankProcess.indexOf(it.pid) > -1) {
        ...
        if (!isTxtTraceType) {
          ...
          if (expectedData.length > 0) {
            ...
            expectedRow.onThreadHandler = (useCache): void => {
              let context: CanvasRenderingContext2D;
              if (expectedRow!.currentContext) {
                context = expectedRow!.currentContext;
              } else {
                context = expectedRow!.collect ? this.trace.canvasFavoritePanelCtx! : this.trace.canvasPanelCtx!;
              }
        ...
        if (actualData.length > 0) {
          ...
          actualRow.onThreadHandler = (useCache): void => {
            let context: CanvasRenderingContext2D;
            if (actualRow!.currentContext) {
              context = actualRow!.currentContext;
            } else {
              context = actualRow!.collect ? this.trace.canvasFavoritePanelCtx! : this.trace.canvasPanelCtx!;
            }
