...
import {
  AppStartupRender,
  AppStartupStruct
} from '../../database/ui-worker/ProcedureWorkerAppStartup';
...
} from '../../database/data-trafic/ProcessDeliverInputEventDataSender';
...
export class SpProcessChart {
  ...
  async init(): Promise<void> {
    ...
    for (let i = 0; i < processList.length; i++) {
      ...
      let renderRow: TraceRow<BaseStruct> | null = null;
      for (let j = 0; j < threads.length; j++) {
        ...
        if (it.processName === 'render_service') {
          ...
          let flag = threads.length === j + 1 && !(this.threadFuncMaxDepthMap.get(`${thread.upid}-${thread.tid}`) !== undefined);
                  ...
                  if (funs.length > 0) {
                    funs.forEach((fun, index) => {
                      ...
                      if (Utils.isBinder(fun)) {
                        ...
                        if (fun.dur === -1 || fun.dur === null || fun.dur === undefined) {
      ...
      processMem.forEach((mem) => {
        ...
        if (renderRow && row.name === 'H:PreferredFrameRate') {
          processRow.addChildTraceRowBefore(row, renderRow);
        } else {
