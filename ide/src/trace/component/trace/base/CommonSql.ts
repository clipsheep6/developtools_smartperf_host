...
const cpuSql: Array<string> = [
  ...
  'SELECT (A.ts - B.start_ts) AS startTs,IFNULL(dur,B.end_ts - A.ts) dur,value FROM measure A,trace_range B;',
  'SELECT ts - T.start_ts AS startNs,dur,max(value) AS max,min(value) AS min FROM measure,trace_range T GROUP BY ts;',
  ...
  'SELECT value, filter_id AS filterId, ts, f.cpu FROM measure LEFT JOIN cpu_measure_filter AS f ON f.id=filter_id ' +
  'ORDER BY ts ASC;',
];

const threadSql: Array<string> = [
...
const memory: Array<string> = [
  'SELECT startNs, SUM( value ) AS value FROM ( SELECT m.ts - tr.start_ts AS startNs,SUM( m.value ) AS value FROM' +
  ' sys_mem_measure m,trace_range tr LEFT JOIN sys_event_filter f ON f.id = m.filter_id WHERE m.ts < tr.end_ts ' +
  ...
  'FROM smaps A,trace_range B WHERE A.timestamp < B.end_ts GROUP BY A.timestamp;',
  'SELECT (A.timestamp - B.start_ts) AS startNs, SUM(private_clean + private_dirty) * 1024 AS value,\'private_clean ' +
  '+ private_dirty\' AS name FROM smaps A,trace_range B WHERE A.timestamp < B.end_ts GROUP BY A.timestamp;',
  ...
  'SELECT (A.ts - B.start_ts) AS startNs,SUM(A.size) AS value,A.flag AS flag,A.ipid AS ipid,E.data AS expTaskComm ' +
  'FROM memory_dma A,trace_range B LEFT JOIN data_dict AS E ON E.id=A.exp_task_comm_id ' +
...
const bioSql: Array<string> = [
  ...
  'SELECT (A.start_ts -B.start_ts) AS startNS,(A.start_ts - B.start_ts + A.latency_dur) AS endNS,latency_dur AS dur ' +
  'FROM bio_latency_sample A,trace_range B WHERE startNS > 0 ORDER BY A.start_ts;'
];

const frameTimeSql: Array<string> = [
  'SELECT sf.id,\'frameTime\' AS frame_type,fs.ipid,fs.vsync AS name,fs.dur AS app_dur,(sf.ts + sf.dur - fs.ts) AS ' +
  'dur,(fs.ts - TR.start_ts) AS ts,fs.type,fs.flag,pro.pid,pro.name AS cmdline,(sf.ts - TR.start_ts) AS rs_ts,' +
  'sf.vsync AS rs_vsync,sf.dur AS rs_dur,sf.ipid AS rs_ipid,proc.pid AS rs_pid,proc.name AS rs_name ' +
  'FROM frame_slice AS fs LEFT JOIN process AS pro ON pro.id = fs.ipid LEFT JOIN frame_slice AS sf ON fs.dst = sf.id ' +
  'LEFT JOIN process AS proc ON proc.id = sf.ipid LEFT JOIN trace_range TR WHERE fs.dst IS NOT NULL AND fs.type = 1 ' +
  'UNION SELECT -1 AS id,\'frameTime\' AS frame_type,fs.ipid,fs.vsync  AS name,fs.dur AS app_dur,fs.dur,' +
  ...
  'SELECT sf.id,\'frameTime\' AS frame_type,fs.ipid,fs.vsync AS name,fs.dur AS app_dur,(sf.ts + sf.dur - fs.ts) AS' +
  ' dur,(fs.ts - TR.start_ts) AS ts,fs.type,(CASE WHEN (sf.flag == 1 OR fs.flag == 1) THEN 1  WHEN ' +
  '(sf.flag == 3 OR fs.flag == 3 ) THEN 3 ELSE 0 end) AS jank_tag,pro.pid,pro.name AS cmdline,(sf.ts - TR.start_ts) ' +
  'AS rs_ts,sf.vsync AS rs_vsync,sf.dur AS rs_dur,sf.ipid AS rs_ipid,proc.pid AS rs_pid, proc.name AS rs_name ' +
  'FROM frame_slice AS fs LEFT JOIN process AS pro ON pro.id = fs.ipid LEFT JOIN frame_slice AS sf ON fs.dst = sf.id ' +
  'LEFT JOIN process AS proc ON proc.id = sf.ipid LEFT JOIN trace_range TR WHERE fs.dst IS NOT NULL AND fs.type = 0 ' +
  'AND fs.flag <> 2 UNION SELECT -1 AS id,\'frameTime\' AS frame_type,fs.ipid,fs.vsync AS name,fs.dur AS app_dur,' +
  'fs.dur,(fs.ts - TR.start_ts) AS ts,fs.type,fs.flag AS jank_tag,pro.pid,pro.name AS cmdline,NULL AS rs_ts,' +
  'NULL AS rs_vsync,NULL AS rs_dur,NULL AS rs_ipid,NULL AS rs_pid,NULL AS rs_name FROM frame_slice AS fs LEFT JOIN ' +
  'process AS pro ON pro.id = fs.ipid LEFT JOIN trace_range TR WHERE fs.dst IS NULL AND pro.name NOT LIKE ' +
  '\'%render_service%\' AND fs.type = 0 AND fs.flag <> 2 ORDER BY ts;',
];

const appStartUpSql: Array<string> = [
  'SELECT P.pid,A.tid,A.call_id AS itid,(CASE WHEN A.start_time < B.start_ts THEN 0 ELSE (A.start_time - B.start_ts) ' +
  'end) AS startTs,(CASE WHEN A.start_time < B.start_ts THEN (A.end_time - B.start_ts) WHEN A.end_time = -1 THEN' +
  ' 0 ELSE (A.end_time - A.start_time) end) AS dur,A.start_name AS startName FROM app_startup A,trace_range B ' +
  'LEFT JOIN process P ON A.ipid = P.ipid ORDER BY start_name;'
];

const animationSql: Array<string> = [
  'SELECT a.id AS animationId,(CASE WHEN a.input_time NOT NULL THEN (a.input_time - R.start_ts) ELSE ' +
  '(a.start_point- R.start_ts) END) AS ts,(a.start_point - R.start_ts) AS dynamicStartTs,(a.end_point - R.start_ts) ' +
  'AS dynamicEndTs FROM animation AS a,trace_range AS R ORDER BY ts;',
  ...
  'SELECT d.id,d.width AS currentFrameWidth,d.height AS currentFrameHeight,d.name AS nameId,(d.end_time - R.start_ts)' +
  ' AS currentTs,d.x,d.y FROM dynamic_frame AS d,trace_range AS R ORDER BY d.end_time;',
];

export function getAllSql(): Array<string> {
