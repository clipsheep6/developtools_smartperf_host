...
const cpuSql: Array<DebugSql> = [{
  sql: 'SELECT B.pid AS processId,B.cpu,B.tid,B.itid AS id,B.dur,B.ts - TR.start_ts AS startTime,B.arg_setid AS ' +
    'argSetID FROM thread_state AS B LEFT JOIN trace_range AS TR WHERE B.itid IS NOT NULL;',
  title: 'Cpu chart'
},{
  sql: 'SELECT cpu,value,IFNULL(dur,tb.end_ts - c.ts) dur,ts-tb.start_ts AS startNS FROM measure c,trace_range tb ' +
  ...
  sql: 'SELECT ts - T.start_ts AS startNs,dur,max(value) AS max,min(value) AS min FROM measure,trace_range ' +
    'T GROUP BY ts;',
  title: 'Cpu Freq Limit chart'
},{
  sql: 'SELECT TS.pid AS pid,TS.tid AS tid,TS.cpu,SUM((TS.ts - TR.start_ts + TS.dur) - (TS.ts - TR.start_ts)) ' +
  ...
  sql: 'SELECT B.pid AS pid, SUM(B.dur) AS wallDuration,AVG(B.dur) AS avgDuration,count(B.tid) AS occurrences FROM ' +
    'thread_state AS B LEFT JOIN trace_range AS TR GROUP BY B.pid ORDER BY wallDuration DESC;',
  title: 'CPU By Process table'
},{
  sql: 'SELECT cpu,SUM(A.dur) / CAST(B.end_ts - B.start_ts AS float) AS usage FROM thread_state A,trace_range B ' +
    'WHERE (A.ts - B.start_ts) > 0 AND A.dur > 0 GROUP BY cpu;',
  title: 'CPU Usage table'
},{
  sql: 'SELECT B.pid,B.tid,B.state,SUM(B.dur) AS wallDuration,AVG(IFNULL(B.dur,0)) AS ' +
  ...
  sql: 'SELECT value, filter_id AS filterId, ts, f.cpu FROM measure LEFT JOIN cpu_measure_filter AS f ON ' +
    'f.id=filter_id ORDER BY ts ASC;',
  title: 'Cpu State/Cpu Frequency table'
}];

const threadSql: Array<DebugSql> = [{
  sql: 'SELECT  ta.cpu,dur, ts-TR.start_ts AS startTime FROM thread_state ta LEFT JOIN trace_range AS TR ' +
    'WHERE ta.cpu IS NOT NULL;',
  title: 'Process chart'
}];

const memory: Array<DebugSql> = [{
  sql: 'SELECT startNs, SUM( value ) AS value FROM ( SELECT m.ts - tr.start_ts AS startNs,SUM( m.value ) AS value ' +
  ...
  sql: 'SELECT (A.ts - B.start_ts) AS startNs,SUM(A.size) AS value,E.data AS expTaskComm, A.flag AS flag ' +
  ...
  sql: 'SELECT (A.ts - B.start_ts) AS startNs, SUM(A.used_gpu_size) AS value FROM memory_process_gpu A,trace_range B ' +
    'WHERE A.ts < B.end_ts GROUP BY A.ts;',
  title: 'Ability Monitor gpu memory chart'
},{
  sql: 'SELECT (A.timestamp - B.start_ts) AS startNs, SUM(dirty) * 1024 AS value, \'dirty\' AS name ' +
    'FROM smaps A,trace_range B WHERE A.timestamp < B.end_ts GROUP BY A.timestamp;',
  title: 'VM Tracker Smaps chart(dirty)'
},{
  sql: 'SELECT (A.timestamp - B.start_ts) AS startNs, SUM(swapped) * 1024 AS value, \'swapped\' AS name ' +
    'FROM smaps A,trace_range B WHERE A.timestamp < B.end_ts GROUP BY A.timestamp;',
  title: 'VM Tracker Smaps chart(swapped)'
},{
  sql: 'SELECT (A.timestamp - B.start_ts) AS startNs, SUM(resident_size) * 1024 AS value, \'resident_size\' AS name ' +
    'FROM smaps A,trace_range B WHERE A.timestamp < B.end_ts GROUP BY A.timestamp;',
  title: 'VM Tracker Smaps chart(resident_size)'
},{
  sql: 'SELECT (A.timestamp - B.start_ts) AS startNs, SUM(pss) * 1024 AS value, \'pss\' AS name ' +
    'FROM smaps A,trace_range B WHERE A.timestamp < B.end_ts GROUP BY A.timestamp;',
  title: 'VM Tracker Smaps chart(pss)'
},{
  sql: 'SELECT (A.timestamp - B.start_ts) AS startNs, SUM(private_clean + private_dirty) * 1024 AS value,' +
  ...
  sql: 'SELECT (A.ts - B.start_ts) AS startNs,SUM(A.size) AS value FROM memory_ashmem A,trace_range B ' +
    'WHERE A.ts < B.end_ts AND flag = 0 GROUP BY A.ts;',
  title: 'VM Tracker SHM chart'
},{
  sql: 'SELECT (A.ts - B.start_ts) AS startNs,SUM(A.size) AS value,A.flag AS flag,A.ipid AS ipid,E.data AS ' +
  ...
  sql: 'SELECT (ts - start_ts) startNs,SUM(value) value FROM process_measure, trace_range ' +
  ...
  sql: 'SELECT (A.ts - B.start_ts) AS startNs,SUM(A.used_gpu_size) AS value,A.ipid AS ipid ' +
    'FROM memory_process_gpu A,trace_range B WHERE A.ts < B.end_ts GROUP BY A.ts;',
  title: 'VM Tracker gpu memory chart'
},{
  sql: 'SELECT DISTINCT module_name_id id,data FROM memory_window_gpu A, trace_range TR LEFT JOIN data_dict B ' +
    'ON A.module_name_id = B.id WHERE window_name_id = 0 AND A.ts < TR.end_ts;',
  title: 'VM Tracker gpu total type chart'
},{
  sql: 'SELECT DISTINCT A.window_name_id AS id,B.data, NULL AS pid FROM memory_window_gpu A, trace_range tr ' +
...
const bioSql: Array<DebugSql> = [{
  sql: 'SELECT name,B.ipid,pid FROM (SELECT DISTINCT ipid FROM bio_latency_sample A,trace_range B WHERE A.start_ts ' +
    'BETWEEN B.start_ts AND B.end_ts) A LEFT JOIN process B ON A.ipid = B.ipid;',
  title: 'Disk IO process chart'
}, {
  sql: 'SELECT (A.start_ts -B.start_ts) AS startNS,(A.start_ts - B.start_ts + A.latency_dur) AS endNS,latency_dur ' +
...
const frameTimeSql: Array<DebugSql> = [{
  sql: 'SELECT sf.id,\'frameTime\' AS frame_type,fs.ipid,fs.vsync AS name,fs.dur AS app_dur,(sf.ts + sf.dur - fs.ts) ' +
...
const appStartUpSql: Array<DebugSql> = [{
  sql: 'SELECT P.pid,A.tid,A.call_id AS itid,(CASE WHEN A.start_time < B.start_ts THEN 0 ELSE (A.start_time - ' +
...
const animationSql: Array<DebugSql> = [{
  sql: 'SELECT a.id AS animationId,(CASE WHEN a.input_time NOT NULL THEN (a.input_time - R.start_ts) ' +
  ...
  sql: 'SELECT d.id,d.x,d.y,d.width,d.height,d.alpha,d.name AS appName,(d.end_time - R.start_ts) AS ts ' +
    'FROM dynamic_frame AS d,trace_range AS R ORDER BY d.end_time;',
  title: 'Animation effect curve chart'
},
{
  sql: 'SELECT d.id,d.width AS currentFrameWidth,d.height AS currentFrameHeight,d.name AS nameId,(d.end_time ' +
