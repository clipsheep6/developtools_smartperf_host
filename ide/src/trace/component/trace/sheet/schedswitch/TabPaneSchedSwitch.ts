...
export class TabPaneSchedSwitch extends BaseElement {
    ...
    checkInputRangeFn(
        ...
        if ((thirdInput!.value === '' && fourInput!.value !== '') || (thirdInput!.value !== '' && fourInput!.value === '')) {
            this.queryButton!.style.pointerEvents = 'none';
            this.isQueryButtonClick(false);
        }
    }
    ...
    async singleCutLogic(threadFunName: string, threadId: string, leftStartNs: number, rightEndNs: number): Promise<void> {
    ...
    queryCycleRangeData(): void {
        ...
        if (cycleALeft !== '' && cycleARight !== '' && cycleALeft !== cycleARight) {
            let countA: number = 0;
            let rangeFilterA: Array<TreeSwitchConfig> = this.clickThreadChildren.filter((it: TreeSwitchConfig) =>
                Number(it.duration) >= Number(cycleALeft) &&
                Number(it.duration) < Number(cycleARight)
            );
        ...
        if (cycleBLeft !== '' && cycleBRight !== '' && cycleBLeft !== cycleBRight) {
            let countB: number = 0;
            let rangeFilterB: Array<TreeSwitchConfig> = this.clickThreadChildren.filter((it: TreeSwitchConfig) =>
                Number(it.duration) >= Number(cycleBLeft) &&
                Number(it.duration) < Number(cycleBRight)
            );
