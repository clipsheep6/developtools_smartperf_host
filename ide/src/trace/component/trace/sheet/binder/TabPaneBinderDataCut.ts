...
import { BinderItem, BinderGroup, DataSource, FuncNameCycle, BinderDataStruct } from '../../../../bean/BinderProcessThread';
            ...
            if (this.funcNameCycleArr.length !== 0) {
                ...
                for (let j: number = 0; j < this.funcNameCycleArr.length - 1; j++) {
                    ...
                    for (let i: number = 0; i < binderItemArr.length; i++) {
                        if (binderItemArr[i].ts > this.funcNameCycleArr[j].cycleStartTime
                            && binderItemArr[i].ts + binderItemArr[i].dur < this.funcNameCycleArr[j + 1].cycleStartTime) {
                            // calculate cycle duration
                            binderItemArr[i].cycleDur = this.funcNameCycleArr[j + 1].cycleStartTime - this.funcNameCycleArr[j].cycleStartTime;
        ...
        if (threadIdValue !== '' && threadFuncName !== '') {
            ...
            if (this.funcNameCycleArr.length !== 0) {
                ...
                for (let j: number = 0; j < this.funcNameCycleArr.length; j++) {
                    for (let i: number = 0; i < binderItemArr.length; i++) {
                        if (binderItemArr[i].ts > this.funcNameCycleArr[j].cycleStartTime
                            && binderItemArr[i].ts + binderItemArr[i].dur < this.funcNameCycleArr[j].cycleStartTime + this.funcNameCycleArr[j]!.cycleDur) {
