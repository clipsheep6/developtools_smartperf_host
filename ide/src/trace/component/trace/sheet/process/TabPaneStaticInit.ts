...
export class TabPaneStaticInit extends BaseElement {
  ...
  set data(staticinitParam: SelectionParam | unknown) {
    ...
    getTabStaticInit(staticinitParam.processIds, staticinitParam.leftNs, staticinitParam.rightNs).then(//@ts-ignore
      (result: SoStruct[]) => {
  ...
  initElements(): void {
    ...
    this.staticinitTbl!.addEventListener('column-click', (evt: unknown) => {// @ts-ignore
  ...
  sortByColumn(soDetail: unknown): void {
    let compare = (soA: SoTreeItem, soB: SoTreeItem): number =>// @ts-ignore
