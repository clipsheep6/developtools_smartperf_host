...
export class TabPaneStartup extends BaseElement {
  ...
  set data(startupParam: SelectionParam | unknown) {
    ...
    getTabStartups(startupParam.processIds, startupParam.leftNs, startupParam.rightNs).then(//@ts-ignore
  ...
  initElements(): void {
    ...
    this.startupTbl!.addEventListener('column-click', (evt: unknown) => {// @ts-ignore
  ...
  sortByColumn(startupDetail: unknown): void {
    let compare = (startupA: StartupTreeItem, startupB: StartupTreeItem): number => {// @ts-ignore
