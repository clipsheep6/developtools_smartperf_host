...
@element('tabpane-hang-counter')
export class TabPaneHangCounter extends BaseElement {
  private hangCounterTbl: LitTable | null | undefined;
  private hangCounterRange: HTMLLabelElement | null | undefined;
  private hangCounterSource: Array<SelectionData> = [];

  set data(hangCounterValue: SelectionParam) {
    //@ts-ignore
    this.hangCounterTbl?.shadowRoot?.querySelector('.table')?.style?.height = `${
      this.parentElement!.clientHeight - 45
    }px`;
    this.hangCounterRange!.textContent = `Selected range: ${parseFloat(
      ((hangCounterValue.rightNs - hangCounterValue.leftNs) / 1000000.0).toFixed(5)
    )} ms`;
    this.getCounterData(hangCounterValue).then();
  }

  async getCounterData(hangCounterValue: SelectionParam): Promise<void> {
    let dataSource: Array<SelectionData> = [];
    let collect = hangCounterValue.hangMapData;
    let sumCount = 0;
    this.hangCounterTbl!.loading = true;
    for (let key of collect.keys()) {
      let counters = collect.get(key);
      let res = await counters?.({
        startNS: hangCounterValue.leftNs,
        endNS: hangCounterValue.rightNs,
        queryAll: true,
      });
      let sd = this.createSelectCounterData(key, res || [], hangCounterValue.leftNs, hangCounterValue.rightNs);
    ...
    this.hangCounterTbl!.loading = false;
    this.hangCounterSource = dataSource;
    this.hangCounterTbl!.recycleDataSource = dataSource;
  }

  initElements(): void {
    this.hangCounterTbl = this.shadowRoot?.querySelector<LitTable>('#tb-counter');
    this.hangCounterRange = this.shadowRoot?.querySelector('#time-range');
    this.hangCounterTbl!.addEventListener('column-click', (evt): void => {
      // @ts-ignore
  ...
  connectedCallback(): void {
    ...
    resizeObserver(this.parentElement!, this.hangCounterTbl!);
  }
  ...
  initHtml(): string {
    return `
        ...
        <label id="time-range" class="hang-counter-label" style="width: 100%;height: 20px;text-align: end;font-size: 10pt;">Selected range:0.0 ms</label>
  ...
  sortByColumn(detail: unknown): void {
    ...
    function compare(property, sort, type) {
      return function (hangCounterLeftData: SelectionData, hangCounterRightData: SelectionData): number {
        if (hangCounterLeftData.process === ' ' || hangCounterRightData.process === ' ') {
          return 0;
        }
        if (type === 'number') {
          return sort === 2 // @ts-ignore
            ? parseFloat(hangCounterRightData[property]) - parseFloat(hangCounterLeftData[property]) // @ts-ignore
            : parseFloat(hangCounterLeftData[property]) - parseFloat(hangCounterRightData[property]);
        } else {
          // @ts-ignore
          if (hangCounterRightData[property] > hangCounterLeftData[property]) {
            return sort === 2 ? 1 : -1;
          } else {
            // @ts-ignore
            if (hangCounterRightData[property] === hangCounterLeftData[property]) {
    ...
    if (detail.key === 'name') {
      // @ts-ignore
      this.hangCounterSource.sort(compare(detail.key, detail.sort, 'string'));
    } else {
      // @ts-ignore
      this.hangCounterSource.sort(compare(detail.key, detail.sort, 'number'));
    }
    this.hangCounterTbl!.recycleDataSource = this.hangCounterSource;
  }
}
