...
export class TabPaneGpufreqDataCut extends BaseElement {
        ...
        this.getGpufreqData(threadStatesParam.leftNs, threadStatesParam.rightNs, false).then((result) => {
            if (result !== null && result.length > 0) {
                ...
                if (result.length === 1) {
                    ...
                    resultList[resultList.length - 1].dur = String(threadStatesParam.rightNs - Number(resultList[resultList.length - 1].startNS));
    ...
    initElements(): void {
        ...
        this._single?.addEventListener('click', (e) => {
            ...
            this.threadStatesTbl!.loading = true;
            this.validationFun(
                this.threadIdValue, this.threadFuncName, originalThreadIdStyle,
                originalThreadFuncStyle, originalThreadIdPlaceholder, originalThreadFuncPlaceholder, 'single');
        });
        //点击loop
        this._loop?.addEventListener('click', (e) => {
            ...
            this.validationFun(
                this.threadIdValue, this.threadFuncName, originalThreadIdStyle,
                originalThreadFuncStyle, originalThreadIdPlaceholder,
                originalThreadFuncPlaceholder, 'loop');
        });
    ...
    private handleDataCut(initData: Array<GpuCountBean>, dataCut: Array<SearchGpuFuncBean>): void {
        if (initData.length > 0 && dataCut.length > 0) {
            this.getGpufreqData(this.currentSelectionParam!.leftNs, this.currentSelectionParam!.rightNs, true)
                .then((result: Array<GpuCountBean>) => {
    ...
    private filterData(initData: Array<GpuCountBean>, dataCut: Array<SearchGpuFuncBean>, result: Array<GpuCountBean>): void {
    ...
    private segmentationData(j: GpuCountBean, e: SearchGpuFuncBean, i: number): Array<GpuCountBean> {
        ...
        if (e.startTime >= Number(j.startNS) && e.startTime <= Number(j.endTime)) {
            if (e.endTime <= Number(j.endTime)) {
                lastList.push(new GpuCountBean(
                    j.filterId, j.freq,
    ...
    private updateValueMap(item: GpuCountBean, parentIndex: number, freq: string, valueMap: { [parentIndex: string]: TreeDataBean }, UNIT: number): void {
