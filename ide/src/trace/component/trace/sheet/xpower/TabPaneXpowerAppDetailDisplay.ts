...
export class TabPaneXpowerDisplay extends BaseElement {
  ...
  createSelectCounterData(list: number[], key: string): SelectionData {
    ...
    if (list.length > 0) {
      ...
      selectCounterData.max = Utils.timeFormat(max);
      selectCounterData.minNumber = min;
      selectCounterData.min = Utils.timeFormat(min);
      selectCounterData.avgNumber = Number((total / list.length).toFixed(2));
      selectCounterData.average = Utils.timeFormat(Number((total / list.length).toFixed(2)));
    }
  ...
  sortByColumn(detail: { key: string; sort: number }): void {
    ...
    if (detail.key === 'name') {
      ...
      this.xpowerCounterSource.sort(compare(detail.key, detail.sort, 'min'));
    } else if (detail.key === 'average') {
      this.xpowerCounterSource.sort(compare(detail.key, detail.sort, 'average'));
    } else {
