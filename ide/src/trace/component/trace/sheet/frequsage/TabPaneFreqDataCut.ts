...
export class TabPaneFreqDataCut extends BaseElement {
      ...
      for (let e of result) {
        if (processArr.includes(e.pid)) {
          ...
          let arr: Array<{
            ...
            process: string; 
            thread: string;
          }> | undefined = needDeal.get(e.pid + '_' + e.tid);
          sum += e.dur;
          arr?.push(e);
        }
      }
    ...
    if (threadIdValue !== '' && threadFuncName !== '') {
      ...
      querySearchFuncData(threadFuncName, Number(threadIdValue), leftNS, rightNS).then((result) => {
        if (result !== null && result.length > 0) {
          ...
          for (let i of result) {
            if (
              i.startTime + this.currentSelectionParam.recordStartNs + i.dur <
              this.currentSelectionParam.rightNs + this.currentSelectionParam.recordStartNs) {
      ...
      for (let i = 0; i < dealArr.length; i++) {
        ...
        cycleMap.get(key)?.push(
            new TabPaneFreqUsageConfig('cycle' + (i + 1) + '—' + value[0].thread,
              ((dealArr[i].ts - timeDur) / 1000000).toFixed(3), key.split('_')[0], key.split('_')[1],
              0, '', '', 0, (dealArr[i].dur / 1000000).toFixed(3), 0, 'cycle', i + 1, [] ));
        for (let j = 0; j < value.length; j++) {
          // 判断若用户导入json文件，则替换为对应cpu下的对应频点的算力值进行算力消耗计算
          // @ts-ignore
          let consumptionMap: Map<number, number> =
            SpSegmentationChart.freqInfoMapData.size > 0 && SpSegmentationChart.freqInfoMapData.get(value[j].cpu);
          // 若存在算力值，则直接取值做计算。若不存在算力值，且频点值不为unknown的情况，则取频点值做计算，若为unknown，则取0做兼容
          const consumption: number = consumptionMap && consumptionMap.get(value[j].freq)
              ? consumptionMap.get(value[j].freq) : (value[j].freq === 'unknown' ? 0 : value[j].freq);
          if (!cpuArr.includes(value[j].cpu)) {
            cpuArr.push(value[j].cpu);
            cpuMap.get(key)?.push(
                new TabPaneFreqUsageConfig('cycle' + (i + 1) + '—' + value[j].thread, '',
          ...
          if (dealArr[i].ts < value[j].ts &&
            dealArr[i].ts + dealArr[i].dur > value[j].ts &&
            dealArr[i].ts + dealArr[i].dur > value[j].ts + value[j].dur) {
              resList.push(new TabPaneFreqUsageConfig('cycle' + (i + 1) + '—' + value[j].thread, '',
                  value[j].pid, value[j].tid, (consumption * value[j].dur) / countMutiple, value[j].cpu,
                  value[j].freq, value[j].dur, '', value[j].percent, 'freqdata', i, undefined));
              totalList.get(key)?.push(new TabPaneFreqUsageConfig(
                    value[j].thread, '', value[j].pid, value[j].tid, (consumption * value[j].dur) / countMutiple,
                    value[j].cpu, value[j].freq, value[j].dur, '', value[j].percent, 'freqdata', i, undefined));
          }
          ...
          if (dealArr[i].ts < value[j].ts &&
            ...
            dealArr[i].ts + dealArr[i].dur <= value[j].ts + value[j].dur) {
              resList.push(new TabPaneFreqUsageConfig('cycle' + (i + 1) + '—' + value[j].thread,'', 
                value[j].pid, value[j].tid, ((dealArr[i].ts + dealArr[i].dur - value[j].ts) * consumption) / countMutiple,
                value[j].cpu, value[j].freq, dealArr[i].ts + dealArr[i].dur - value[j].ts, '',
                ((dealArr[i].ts + dealArr[i].dur - value[j].ts) / value[j].dur) * value[j].percent,
                'freqdata', i, undefined));
              totalList.get(key)?.push(new TabPaneFreqUsageConfig(value[j].thread, '', value[j].pid, value[j].tid,
                ((dealArr[i].ts + dealArr[i].dur - value[j].ts) * consumption) / countMutiple, value[j].cpu,
                value[j].freq, dealArr[i].ts + dealArr[i].dur - value[j].ts, '',
                ((dealArr[i].ts + dealArr[i].dur - value[j].ts) / value[j].dur) * value[j].percent,
                'freqdata', i, undefined));
              break;
          }
          ...
          if (dealArr[i].ts > value[j].ts &&
            value[j].ts + value[j].dur > dealArr[i].ts &&
            value[j].ts + value[j].dur > dealArr[i].ts + dealArr[i].dur) {
              resList.push(new TabPaneFreqUsageConfig('cycle' + (i + 1) + '—' + value[j].thread,
                '', value[j].pid, value[j].tid, (dealArr[i].dur * consumption) / countMutiple, value[j].cpu,
                value[j].freq, dealArr[i].dur, '', (dealArr[i].dur / value[j].dur) * value[j].percent, 
                'freqdata', i, undefined));
              totalList.get(key)?.push(new TabPaneFreqUsageConfig(value[j].thread, '', value[j].pid, value[j].tid,
                (dealArr[i].dur * consumption) / countMutiple, value[j].cpu, value[j].freq, dealArr[i].dur, '',
                (dealArr[i].dur / value[j].dur) * value[j].percent, 'freqdata', i, undefined));
              break;
          }
          ...
          if (dealArr[i].ts > value[j].ts &&
            ...
            value[j].ts + value[j].dur <= dealArr[i].ts + dealArr[i].dur) {
              resList.push(new TabPaneFreqUsageConfig('cycle' + (i + 1) + '—' + value[j].thread,'', 
                value[j].pid, value[j].tid, ((value[j].ts + value[j].dur - dealArr[i].ts) * consumption) / countMutiple, 
                value[j].cpu, value[j].freq, value[j].ts + value[j].dur - dealArr[i].ts, '',
                ((value[j].ts + value[j].dur - dealArr[i].ts) / value[j].dur) * value[j].percent, 'freqdata', i, undefined));
              totalList.get(key)?.push(new TabPaneFreqUsageConfig(value[j].thread, '', value[j].pid, value[j].tid,
                ((value[j].ts + value[j].dur - dealArr[i].ts) * consumption) / countMutiple, value[j].cpu, value[j].freq,
                value[j].ts + value[j].dur - dealArr[i].ts, '', ((value[j].ts + value[j].dur - dealArr[i].ts) / value[j].dur) *
                  value[j].percent, 'freqdata', i, undefined));
          }
        }
        ...
        if (res !== null && res.length > 0) {
          ...
          for (let i of res) {
            cutArr[cutArr.length - 1] && (cutArr[cutArr.length - 1].dur = i.startTime ? i.startTime +
                  this.currentSelectionParam.recordStartNs - cutArr[cutArr.length - 1].ts : 0);
  ...
  mergeLoopData(
    ...
    targetMap: Map<string, Array<any>>,
    cycleMap: Map<string, Array<TabPaneFreqUsageConfig>>,
    totalList: Map<string, Array<TabPaneFreqUsageConfig>>
  ): void {
    ...
    targetMap.forEach((value, key) => {
      ...
      for (let i = 0; i < cutArr.length - 1; i++) {
        ...
        cycleMap.get(key)?.push(new TabPaneFreqUsageConfig('cycle' + (i + 1) + '—' + value[0].thread,
          ((cutArr[i].ts - timeDur) / 1000000).toFixed(3), key.split('_')[0], key.split('_')[1], 0, '',
          '', 0, (cutArr[i].dur / 1000000).toFixed(3), 0, 'cycle', i + 1, []));
        for (let j = 0; j < value.length; j++) {
          // 判断若用户导入json文件，则替换为对应cpu下的对应频点的算力值进行算力消耗计算
          // @ts-ignore
          let consumptionMap: Map<number, number> =
          ...
          if (!cpuArr.includes(value[j].cpu)) {
            cpuArr.push(value[j].cpu);
            // 创建cpu层级数据，以便后续生成树结构
            cpuMap.get(key)?.push(new TabPaneFreqUsageConfig('cycle' + (i + 1) + '—' + value[j].thread,
              '', value[j].pid, value[j].tid, 0, value[j].cpu, '', 0, '', 0, 'cpu', -1, []));
          }
          ...
          if (value[j].ts >= cutArr[i].ts && value[j].ts + value[j].dur <= cutArr[i + 1].ts) {
            resList.push(new TabPaneFreqUsageConfig('cycle' + (i + 1) + '—' + value[j].thread, '',
              value[j].pid, value[j].tid, (consumption * value[j].dur) / countMutiple, value[j].cpu,
              value[j].freq, value[j].dur, '', value[j].percent, 'freqdata', i, undefined));
            totalList.get(key)?.push(new TabPaneFreqUsageConfig(value[j].thread, '', value[j].pid,
              value[j].tid, (consumption * value[j].dur) / countMutiple, value[j].cpu, value[j].freq,
              value[j].dur, '', value[j].percent, 'freqdata', i, undefined));
          }
          // 如果频点数据开始时间大于某一周期起始时间，且结束时间大于下一同名方法开始时间的情况
          if (value[j].ts >= cutArr[i].ts && value[j].ts + value[j].dur > cutArr[i + 1].ts) {
            if (cutArr[i + 1].ts - value[j].ts > 0) {
              resList.push(new TabPaneFreqUsageConfig('cycle' + (i + 1) + '—' + value[j].thread, '',
                value[j].pid, value[j].tid, (consumption * (cutArr[i + 1].ts - value[j].ts)) / countMutiple,
                value[j].cpu, value[j].freq, cutArr[i + 1].ts - value[j].ts, '',
                  value[j].percent * ((cutArr[i + 1].ts - value[j].ts) / value[j].dur), 'freqdata', i, undefined));
              totalList.get(key)?.push(new TabPaneFreqUsageConfig(value[j].thread, '', value[j].pid, value[j].tid, 
                (consumption * (cutArr[i + 1].ts - value[j].ts)) / countMutiple, value[j].cpu, value[j].freq, 
                cutArr[i + 1].ts - value[j].ts, '', value[j].percent * ((cutArr[i + 1].ts - value[j].ts) / value[j].dur),
                'freqdata', i, undefined));
              break;
            }
          }
          ...
          if (value[j].ts < cutArr[i].ts && value[j].ts + value[j].dur > cutArr[i + 1].ts) {
            resList.push(new TabPaneFreqUsageConfig('cycle' + (i + 1) + '—' + value[j].thread,'', value[j].pid,
              value[j].tid, (consumption * (cutArr[i + 1].ts - cutArr[i].ts)) / countMutiple, value[j].cpu, value[j].freq,
              cutArr[i + 1].ts - cutArr[i].ts, '', value[j].percent * ((cutArr[i + 1].ts - cutArr[i].ts) / value[j].dur),
              'freqdata', i, undefined));
            totalList.get(key)?.push(new TabPaneFreqUsageConfig(value[j].thread, '', value[j].pid, value[j].tid,
              (consumption * (cutArr[i + 1].ts - cutArr[i].ts)) / countMutiple, value[j].cpu, value[j].freq,
              cutArr[i + 1].ts - cutArr[i].ts, '', value[j].percent * ((cutArr[i + 1].ts - cutArr[i].ts) / value[j].dur),
              'freqdata', i, undefined));
          }
          // 如果频点数据开始时间小于某一周期起始时间，结束时间大于该方法开始时间。且频点数据结束时间小于下一同名方法开始时间
          if (value[j].ts < cutArr[i].ts &&
            value[j].ts + value[j].dur > cutArr[i].ts && value[j].ts + value[j].dur < cutArr[i + 1].ts) {
            resList.push(new TabPaneFreqUsageConfig('cycle' + (i + 1) + '—' + value[j].thread, '', value[j].pid, value[j].tid,
              (consumption * (value[j].dur + value[j].ts - cutArr[i].ts)) / countMutiple, value[j].cpu, value[j].freq,
              value[j].dur + value[j].ts - cutArr[i].ts, '', value[j].percent *
               ((value[j].dur + value[j].ts - cutArr[i].ts) / value[j].dur), 'freqdata', i, undefined));
            totalList.get(key)?.push(new TabPaneFreqUsageConfig(value[j].thread, '', value[j].pid, value[j].tid,
              (consumption * (value[j].dur + value[j].ts - cutArr[i].ts)) / countMutiple, value[j].cpu, value[j].freq,
              value[j].dur + value[j].ts - cutArr[i].ts, '', value[j].percent * ((value[j].dur + value[j].ts - cutArr[i].ts) /
                value[j].dur), 'freqdata', i, undefined));
          }
        }
        // 合并相同周期内的数据
        this.mergeData(resList);
  ...
  mergeThreadData(
    ...
    for (let i = 0; i < threadArr.length; i++) {
      let cycleMapData: Array<TabPaneFreqUsageConfig> | undefined = cycleMap.get(
  ...
  setConfig(
    ...
    this.statisticsScatter!.config = {
      ...
      yAxisLabel: [
        Math.round(yAxis / 5),
        Math.round((yAxis * 2) / 5),
        Math.round((yAxis * 3) / 5),
        Math.round((yAxis * 4) / 5),
        Math.round(yAxis),
      ],
      // 横轴坐标值
      xAxisLabel: [
        Math.round(xAxis / 5),
        Math.round((xAxis * 2) / 5),
        Math.round((xAxis * 3) / 5),
        Math.round((xAxis * 4) / 5),
        Math.round(xAxis),
        Math.round((xAxis * 6) / 5),
      ],
      // 横轴字段、纵轴字段
      AxisLabel: ['负载', '算力供给'],
      // 是否加载最大负载线及均衡线
  ...
  initHtml(): string {
    ...
    <div class="table-box" style="width: 65%; min-width: 65%">
        <lit-table id="tb-running-datacut" style="height: auto; overflow:auto;margin-top:5px" tree>
            ...
            <div id= "cycleQuery" style="margin-bottom:15px;margin-top:5px;display:none">
                    <div id="cycle-a">
                        <span>Cycle A: </span>
