              ...
              for (let e of result) {
                if (threadStatesParam.processIds.includes(e.pid) && e.state == 'Running') {
                  ...
                  if (!cpuArr.includes(e.cpu)) {
                    cpuArr.push(e.cpu);
                    finalResultArr[0].thread =
                      finalResultArr[0].thread == '' ? e.tid + '_' + e.thread : finalResultArr[0].thread;
                    finalResultArr[0].children.push({
                      thread: e.tid + '_' + e.thread,
                      count: 0,
                      cpu: e.cpu,
                      freq: '',
                      dur: 0,
                    ...
                    if (targetList[i].ts > dealArr[j].startNS) {
                      if (targetList[i].ts < dealArr[j].startNS + dealArr[j].dur) {
                        if (targetList[i].dur < dealArr[j].startNS + dealArr[j].dur - targetList[i].ts) {
                          resultList.push({
                            thread: targetList[i].tid + '_' + targetList[i].thread,
                            count: (dealArr[j].value * targetList[i].dur) / 1000,
                            cpu: targetList[i].cpu,
                            freq: dealArr[j].value,
                            dur: targetList[i].dur,
                            percent: (targetList[i].dur / sum) * 100,
                            state: 'Running',
                            ts: targetList[i].ts / tsMutiple,
                          });
                          break;
                        } else {
                          resultList.push({
                            thread: targetList[i].tid + '_' + targetList[i].thread,
                            count: (dealArr[j].value * (dealArr[j].startNS + dealArr[j].dur - targetList[i].ts)) / 1000,
                            cpu: targetList[i].cpu,
                            freq: dealArr[j].value,
                            dur: dealArr[j].startNS + dealArr[j].dur - targetList[i].ts,
                            percent: ((dealArr[j].startNS + dealArr[j].dur - targetList[i].ts) / sum) * 100,
                            state: 'Running',
                            ts: targetList[i].ts / tsMutiple,
                          });
                        }
                      ...
                      if (targetList[i].ts + targetList[i].dur > dealArr[j].startNS) {
                        if (targetList[i].dur + targetList[i].ts - dealArr[j].startNS < dealArr[j].dur) {
                          resultList.push({
                            thread: targetList[i].tid + '_' + targetList[i].thread,
                            count:
                              (dealArr[j].value * (targetList[i].dur + targetList[i].ts - dealArr[j].startNS)) / 1000,
                            cpu: targetList[i].cpu,
                            freq: dealArr[j].value,
                            dur: targetList[i].dur + targetList[i].ts - dealArr[j].startNS,
                            percent: ((targetList[i].dur + targetList[i].ts - dealArr[j].startNS) / sum) * 100,
                            state: 'Running',
                            ts: dealArr[j].startNS / tsMutiple,
                          });
                          break;
                        } else {
                          resultList.push({
                            thread: targetList[i].tid + '_' + targetList[i].thread,
                            count: (dealArr[j].value * dealArr[j].dur) / 1000,
                            cpu: targetList[i].cpu,
                            freq: dealArr[j].value,
                            dur: dealArr[j].dur,
      ...
      querySearchFuncData(threadFuncName, threadIdValue, leftNS, rightNS).then((res) => {
          ...
          for (let j = 0; j < displayArr.length; j++) {
            ...
            if (displayArr[j].ts >= cutArr[i].ts) {
              if (displayArr[j].ts + displayArr[j].dur <= cutArr[i + 1].ts) {
                finalArr.push({
                  thread: displayArr[j].thread,
                  count: (displayArr[j].freq * displayArr[j].dur) / 1000,
                  cpu: displayArr[j].cpu,
                  freq: displayArr[j].freq,
                  dur: displayArr[j].dur,
                  percent: displayArr[j].percent,
                  state: 'Running',
                  ts: (displayArr[j].ts - timeDur) / tsMutiple,
                  id: i,
                });
              } else {
                if (cutArr[i + 1].ts - displayArr[j].ts > 0) {
                  finalArr.push({
                    thread: displayArr[j].thread,
                    count: (displayArr[j].freq * (cutArr[i + 1].ts - displayArr[j].ts)) / 1000,
                    cpu: displayArr[j].cpu,
                    freq: displayArr[j].freq,
                    dur: cutArr[i + 1].ts - displayArr[j].ts,
                    percent: displayArr[j].percent * ((cutArr[i + 1].ts - displayArr[j].ts) / displayArr[j].dur),
                    state: 'Running',
                    ts: (displayArr[j].ts - timeDur) / tsMutiple,
                    id: i,
                  });
              ...
              if (displayArr[j].ts + displayArr[j].dur > cutArr[i + 1].ts) {
                finalArr.push({
                  thread: displayArr[j].thread,
                  count: (displayArr[j].freq * (cutArr[i + 1].ts - cutArr[i].ts)) / 1000,
                  cpu: displayArr[j].cpu,
                  freq: displayArr[j].freq,
                  dur: cutArr[i + 1].ts - cutArr[i].ts,
                  percent: displayArr[j].percent * ((cutArr[i + 1].ts - cutArr[i].ts) / displayArr[j].dur),
                  state: 'Running',
                  ts: (cutArr[i].ts - timeDur) / tsMutiple,
                  id: i,
                });
              }
              if (
                displayArr[j].ts + displayArr[j].dur > cutArr[i].ts &&
                displayArr[j].ts + displayArr[j].dur < cutArr[i + 1].ts
              ) {
                finalArr.push({
                  thread: displayArr[j].thread,
                  count: (displayArr[j].freq * (displayArr[j].dur + displayArr[j].ts - cutArr[i].ts)) / 1000,
                  cpu: displayArr[j].cpu,
                  freq: displayArr[j].freq,
                  dur: displayArr[j].dur + displayArr[j].ts - cutArr[i].ts,
                  percent:
                    displayArr[j].percent * ((displayArr[j].dur + displayArr[j].ts - cutArr[i].ts) / displayArr[j].dur),
                  state: 'Running',
                  ts: (cutArr[i].ts - timeDur) / tsMutiple,
                  id: i,
                });
        ...
        for (let i = 0; i < finalArr.length; i++) {
          for (let j = i + 1; j < finalArr.length; j++) {
            if (
              finalArr[i].cpu === finalArr[j].cpu &&
              finalArr[i].freq === finalArr[j].freq &&
              finalArr[i].id === finalArr[j].id
            ) {
            ...
            if (dealArr[i].ts < targetList[j].ts) {
              if (dealArr[i].ts + dealArr[i].dur > targetList[j].ts) {
                if (dealArr[i].ts + dealArr[i].dur > targetList[j].ts + targetList[j].dur) {
                  resList.push({
                    thread: targetList[i].thread,
                    ts: (targetList[j].ts - timeDur) / tsMutiple,
                    count: (targetList[j].freq * targetList[j].dur) / 1000,
                    cpu: targetList[j].cpu,
                    freq: targetList[j].freq,
                    dur: targetList[j].dur,
                    percent: targetList[j].percent,
                    state: 'Running',
                    id: i,
                  });
                } else {
                  resList.push({
                    thread: targetList[j].thread,
                    ts: (targetList[j].ts - timeDur) / tsMutiple,
                    count: ((dealArr[i].ts + dealArr[i].dur - targetList[j].ts) * targetList[j].freq) / 1000,
                    cpu: targetList[j].cpu,
                    freq: targetList[j].freq,
                    dur: dealArr[i].ts + dealArr[i].dur - targetList[j].ts,
                    percent:
                      ((dealArr[i].ts + dealArr[i].dur - targetList[j].ts) / targetList[j].dur) * targetList[j].percent,
                    state: 'Running',
                    id: i,
                  });
                  break;
              ...
              if (targetList[j].ts + targetList[j].dur > dealArr[i].ts) {
                if (targetList[j].ts + targetList[j].dur > dealArr[i].ts + dealArr[i].dur) {
                  resList.push({
                    thread: targetList[j].thread,
                    ts: (dealArr[i].ts - timeDur) / tsMutiple,
                    count: (dealArr[i].dur * targetList[j].freq) / 1000,
                    cpu: targetList[j].cpu,
                    freq: targetList[j].freq,
                    dur: dealArr[i].dur,
                    percent: (dealArr[i].dur / targetList[j].dur) * targetList[j].percent,
                    state: 'Running',
                    id: i,
                  });
                  break;
                  ...
                  resList.push({
                    thread: targetList[j].thread,
                    ts: (dealArr[i].ts - timeDur) / tsMutiple,
                    count: ((targetList[j].ts + targetList[j].dur - dealArr[i].ts) * targetList[j].freq) / 1000,
                    cpu: targetList[j].cpu,
                    freq: targetList[j].freq,
                    dur: targetList[j].ts + targetList[j].dur - dealArr[i].ts,
                    percent:
                      ((targetList[j].ts + targetList[j].dur - dealArr[i].ts) / targetList[j].dur) *
                      targetList[j].percent,
                    state: 'Running',
                    id: i,
                  });
                }
      ...
      for (let j = 0; j < arr2.length; j++) {
        if (arr1.children[i].id == arr2[j].id) {
          if (!cpuArr.includes(arr2[j].cpu)) {
            ...
            arr1.children[i].children.push({
              thread: arr2[j].thread,
              count: 0,
              cpu: arr2[j].cpu,
              freq: '',
              dur: 0,
            ...
            if (
              arr1.children[i].children[flag].cpu == arr2[j].cpu &&
              arr1.children[i].children[flag].id == arr2[j].id
            ) {
            ...
            if (
              arr1.children[i].children[count].cpu == arr2[j].cpu &&
              arr1.children[i].children[count].id == arr2[j].id
            ) {
