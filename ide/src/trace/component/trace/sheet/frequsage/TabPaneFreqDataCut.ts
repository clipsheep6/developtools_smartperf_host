...
export class TabPaneFreqDataCut extends BaseElement {
    ...
    initHtml(): string {
        ...
        <lit-table id="tb-running-percent" style="height: auto; overflow-x:auto;width:calc(100vw - 270px);margin-top:5px" tree>
            ...
            <lit-table-column class="running-percent-column" width="240px" title="周期起始时间(ms)" data-index="ts" key="ts" align="flex-start">
            </lit-table-column>
            ...
            <lit-table-column class="running-percent-column" width="120px" title="Freq(KHz)" data-index="freq" key="freq" align="flex-start">
            </lit-table-column>
            <lit-table-column class="running-percent-column" width="200px" title="Running_dur(ns)" data-index="dur" key="dur" align="flex-start">
            </lit-table-column>
                ...
                if (result != null && result.length > 0) {
                    ...
                    targetMap.forEach((item, key) => {
                        ...
                        for (let i = 0; i < dealArr.length; i++) {
                            ...
                            cycleMap.get(key).push({ 'thread': '周期' + (i + 1) + '—' + item[0].thread, 'ts': ((dealArr[i].ts - timeDur) / 1000000).toFixed(3), 'cdur': (dealArr[i].dur / 1000000).toFixed(3), 'count': 0, 'cpu': '', 'freq': '', 'dur': 0, 'percent': 0, 'state': 'Running', children: new Array() });
        ...
        if (threadIdValue != '' && threadFuncName != '') {
            querySearchFuncData(threadFuncName, Number(threadIdValue), leftNS, rightNS).then(res => {
                if (res != null && res.length > 0) {
                    ...
                    for (let i of res) {
                        cutArr[cutArr.length - 1] && (cutArr[cutArr.length - 1].dur = i.startTime ? i.startTime + timeDur - cutArr[cutArr.length - 1].ts : 0);
                        cutArr.push({ 'ts': i.startTime + timeDur, 'dur': 0 });
                    ...
                    targetMap.forEach((item, key) => {
                        ...
                        for (let i = 0; i < cutArr.length - 1; i++) {
                            ...
                            const countMutiple = 1000;
                            cpuMap.set(key, new Array());
                            cycleMap.get(key).push({ 'thread': '周期' + (i + 1) + '—' + item[0].thread, 'ts': ((cutArr[i].ts - timeDur) / 1000000).toFixed(3), 'cdur': (cutArr[i].dur / 1000000).toFixed(3), 'count': 0, 'cpu': '', 'freq': '', 'dur': 0, 'percent': 0, 'state': 'Running', children: new Array() });
                            let value = JSON.parse(JSON.stringify(item));
                            for (let j = 0; j < value.length; j++) {
                                value[j].ts = value[j].ts * multiple;
                                value[j].freq = value[j].freq == 'unknown' ? 0 : value[j].freq;
