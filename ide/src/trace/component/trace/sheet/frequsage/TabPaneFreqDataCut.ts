        ...
        targetMap.forEach((value, key) => {
            ...
            for (let i = 0; i < dealArr.length; i++) {
                ...
                for (let j = 0; j < value.length; j++) {
                    ...
                    let consumptionMap: Map<number, number> | undefined ;
                    if (SpSegmentationChart.freqInfoMapData.size > 0 ) {
                      consumptionMap = SpSegmentationChart.freqInfoMapData.get(value[j].cpu);
                    }
    ...
    render(res: Array<any>, str: string, queryCycleScatter: Array<number>): void {
        ...
        if (maxFreq.value && maxHz.value) {
            if (/^[0-9]*$/.test(maxFreq.value) && /^[0-9]*$/.test(maxHz.value)) {
                ...
                let freq: Map<number, number> | undefined;
                if (SpSegmentationChart.freqInfoMapData.size > 0){
                  freq = SpSegmentationChart.freqInfoMapData.get(SpSegmentationChart.freqInfoMapData.size - 1)
                }
                // @ts-ignore
                ...
                let scatterArr: Array<Array<number>> = [], traceRowdata: Array<any> = [], cycleA: Array<Array<number>> = [], cycleB: Array<Array<number>> = [];
