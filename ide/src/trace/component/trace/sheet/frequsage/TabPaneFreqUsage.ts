...
import '../../../StackBar.js'
import { getTabRunningPercent } from '../../../../database/SqlLite.js';
import { Utils } from '../../base/Utils.js';
import { log } from '../../../../../log/Log.js';
import { resizeObserver } from '../SheetUtils.js';

@element('tabpane-frequsage')
export class TabePaneFreqUsage extends BaseElement {
    ...
    set data(threadStatesParam: SelectionParam | any) {
        ...
        getTabRunningPercent(threadStatesParam.threadIds, threadStatesParam.leftNs, threadStatesParam.rightNs).then((result) => {
            let [...dealArr] = JSON.parse(JSON.stringify(threadStatesParam)).cpuFreqList;
            if(result != null && result.length > 0){
                log('getTabRunningPercent result size : ' + result.length);
                let sum = 0;
                for(let i of dealArr){
                    if(i.cpu != null){
