    ...
    for (let i = 0; i < item.length; i++) {
      for (let j = 0; j < cpuFreqData.length; j++) {
        if (item[i].cpu == cpuFreqData[j].cpu) {
            ...
            if (item[i].ts > cpuFreqData[j].ts && item[i].ts < cpuFreqData[j].ts + cpuFreqData[j].dur &&
 ...
 * @returns 返回新的对象
 */
function returnObj(item: RunningData, cpuFreqData: CpuFreqData, sum: number, flag: number): RunningFreqData | undefined {
  const PERCENT: number = 100;
  const consumption: number = (SpSegmentationChart.freqInfoMapData.size > 0
    ? SpSegmentationChart.freqInfoMapData.get(item.cpu)?.get(cpuFreqData.value) : cpuFreqData.value)!;
  switch (flag) {
    case 1:
      return {
