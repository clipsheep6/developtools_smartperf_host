  ...
  calInstructionRangeCount(isCycles: boolean) {
    ...
    if (isCycles) {
      instructions = this.onReadableData.reduce((pre: any, current: any) => {
        (pre[`${Math.floor(current.cycles)}`] = pre[`${Math.floor(current.cycles)}`] || []).push(current);
        return pre;
      }, {})
    } else {
      instructions = this.onReadableData.reduce((pre: any, current: any) => {
        (pre[`${Math.floor(current.instructions)}`] = pre[`${Math.floor(current.instructions)}`] || []).push(current);
        return pre;
      }, {})
    }
  ...
  drawMarkers(width: number, height: number) {
    ...
    for (let i = 0; i < 6; i++) {
      if (i === 0) {
        ...
        this.ctx!.fillText(`${parseFloat((i * this.yAvg).toFixed(2))}%`, 30, y);
      }
    }
  }
