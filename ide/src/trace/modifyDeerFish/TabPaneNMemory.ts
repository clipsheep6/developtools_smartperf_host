...
export class TabPaneNMemory extends BaseElement {
  ...
  getDataByNativeMemoryWorker(val: SelectionParam | any, refresh = false): void {
    ...
    this.startNmMemoryWorker('native-memory-queryNativeHookEvent', args, (results: any[]) => {
      ...
      console.log('startNmMemoryWorker44');
      if (results.length > 0) {
        this.memorySource = results;
        this.memoryTbl!.recycleDataSource = this.memorySource;
        console.log('this.memorySource',this.memorySource);
  ...
  setNmMemoryLoading(loading: boolean): void {//xiugai 进度条  
    if (loading) {
      ...
      console.log('setNmMemoryLoading.false');
    } 
  }
          ...
          timeCallback: (timeArr: number[]) => {
            if (timeArr && timeArr.length > 0) {
              ...
              if (filterTemp.length > 0) {
                ...
                TabPaneNMSampleList.addSampleData( //xiugai 2
                  this.rowSelectData,
...
setMallocTableData(result: Array<NativeHookMalloc>, arr: Array<NativeHookStatisticsTableData>, type: number): void {
  result.map((malloc) => {
    ...
    // console.log('Malloc',data.existingValue);
